...
static void ts_chacha20_init_96(struct chacha20_ctx *ctx, const uint8_t *key, const uint8_t *nonce96) {
    ...
    ctx->state[12] = 0;
    ctx->state[13] = U8TO32_LITTLE(nonce96 + 0);
    ctx->state[14] = U8TO32_LITTLE(nonce96 + 4);
    ctx->state[15] = U8TO32_LITTLE(nonce96 + 8);
}
...
static int noise_chacha20poly1305_encrypt(const uint8_t *key, uint64_t nonce,
    ...
    struct chacha20_ctx c_ctx;
    ts_chacha20_init_96(&c_ctx, key, nonce_bytes);

    /* Generate 32-byte Poly1305 key with counter = 0 */
    uint8_t poly_key[64] = {0};
    uint8_t poly_in[64] = {0};
    chacha20(&c_ctx, poly_key, poly_in, 64);

    struct poly1305_context p_ctx;
    poly1305_init(&p_ctx, poly_key);

    /* Encrypt plaintext with counter = 1 */
    chacha20(&c_ctx, ciphertext, plaintext, pt_len);

    /* Poly1305: AD + pad + Ciphertext + pad + len(AD) + len(Ciphertext) */
    static const uint8_t zero[16] = {0};
    if (ad && ad_len > 0) {
        poly1305_update(&p_ctx, ad, ad_len);
        if (ad_len % 16 != 0) {
            poly1305_update(&p_ctx, zero, 16 - (ad_len % 16));
        }
    }
    if (ciphertext && pt_len > 0) {
        poly1305_update(&p_ctx, ciphertext, pt_len);
        if (pt_len % 16 != 0) {
            poly1305_update(&p_ctx, zero, 16 - (pt_len % 16));
        }
    }
    uint8_t lens[16];
    U64TO8_LITTLE(lens, (uint64_t)ad_len);
    U64TO8_LITTLE(lens + 8, (uint64_t)pt_len);
    poly1305_update(&p_ctx, lens, 16);

    /* Append 16-byte tag */
    poly1305_finish(&p_ctx, ciphertext + pt_len);
    return 0;
}
...
static int noise_chacha20poly1305_decrypt(const uint8_t *key, uint64_t nonce,
    ...
    if (ciphertext && payload_len > 0) {
        poly1305_update(&p_ctx, ciphertext, payload_len);
        if (payload_len % 16 != 0) {
            poly1305_update(&p_ctx, zero, 16 - (payload_len % 16));
        }
    }
    uint8_t lens[16];
    U64TO8_LITTLE(lens, (uint64_t)ad_len);
    U64TO8_LITTLE(lens + 8, (uint64_t)payload_len);
    poly1305_update(&p_ctx, lens, 16);

    uint8_t computed_tag[16];
    poly1305_finish(&p_ctx, computed_tag);
    ...
    /* Decrypt ciphertext */
    chacha20(&c_ctx, plaintext, ciphertext, payload_len);
    return 0;
}
